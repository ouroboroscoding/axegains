...
Handles everything related to IATF practice, matches, etc.
"""
...
# Service imports
from services.iatf import IATF
...
}, 'iatf', "https?://(.*\\.)?%s" % Conf.get(("domain","primary")).replace('.', '\\.')).run(
	host=oRestConf['iatf']['host'],
	port=oRestConf['iatf']['port'],
	workers=oRestConf['iatf']['workers']
)
