...
# Record imports
from records.iatf import Match, MatchStats, Practice, PracticePattern, PracticeStats

class IATF(Services.Service):
	"""IATF Service class

	Service for IATF practices and matches
	...
	def match_create(self, data, sesh):
		...
		# Return the ID
		return Services.Response(oMatch['_id'])
	...
	def match_delete(self, data, sesh):
		...
		if not dMatch:
			return Services.Response(error=(1104, 'iatf_match:%s' % data['id']))
		...
		Sync.push('iatf', 'match-%s' % data['id'], {
			"type": "deleted"
		})
	...
	def match_read(self, data, sesh):
		...
		oResponse = Services.read('auth', 'thrower/aliases', {
		...
		# Add the aliases
		dMatch['initiator_alias'] = oResponse.data[dMatch['initiator']]
		dMatch['opponent_alias'] = oResponse.data[dMatch['opponent']]

		# Else return the match
		return Services.Response(dMatch)
	...
	def matchBigaxePoints_update(self, data, sesh):
		...
		if not isinstance(data['throw'], int):
			return Services.Response(error=(1001, [('throw', 'not an int')]))

		# If the clutch is not a bool
		if not isinstance(data['clutch'], bool):
			return Services.Response(error=(1001, [('clutch', 'not a bool')]))

		# If clutch is true but we didn't get 'd', 0, or 7
		if data['clutch']:
			if data['value'] not in ['d', 0, 7]:
				return Services.Response(error=(1001, [('value', 'invalid')]))
		...
		if dMatch['finished']:
			return Services.Response(error=1002)
	...
	def matchStats_read(self, data, sesh):
		...
		Fetches the total stats for all IATF matches
	...
	def matchUnfinished_read(self, data, sesh):
		...
		# Return the matches
		return Services.Response(lMatches)
	...
	def practice_create(self, data, sesh):
		...
		if not isinstance(data['points'], list):
			return Services.Response(error=(1001, [('points', 'must be a list')]))
	...
	def practiceData_read(self, data, sesh):
		...
		if not dPractice:
			return Services.Response(error=(1104, 'practice:%s' % data['id']))

		# Return the data
		return Services.Response(dPractice)
	...
	def practicePattern_create(self, data, sesh):
		...
		# Return the new ID
		return Services.Response(oPattern['_id'])
	...
	def practicePattern_delete(self, data, sesh):
		...
		if not oPattern:
			return Services.Response(error=(1104, 'practice_pattern:%s' % data['id']))
	...
	def practicePattern_update(self, data, sesh):
		...
		if not oPattern:
			return Services.Response(error=(1104, 'practice_pattern:%s' % data['_id']))
		...
		if lErrors:
			return Services.Response(error=(103, lErrors))
	...
	def practiceStats_read(self, data, sesh):
		...
		Fetches the total stats for all IATF practices
